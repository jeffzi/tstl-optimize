 ...
 */
function chain(first: string, ...rest: [string, ...string[]]): tstl.TableIndexExpression {
  let expr: tstl.Expression = id(first);
  for (const part of rest) {
    expr = tstl.createTableIndexExpression(expr, tstl.createStringLiteral(part));
  }
...
describe("statementAssignsToChain (parametrized cases)", () => {
  /**
